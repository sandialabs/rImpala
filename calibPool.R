...
calibPool <- function(setup){
	...
	good = setup$checkConstraints(tran_unif(theta_start, setup$bounds_mat, names(setup$bounds)), setup$bounds)

	while (any(!good)){
		theta_start[!good, ] = matrix(runif(sum(!good)*setup$p),sum(!good))
		good[!good] = setup$checkConstraints(tran_unif(theta_start[!good, ], setup$bounds_mat, names(setup$bounds)), setup$bounds)
	}
	...
	pb <- progress_bar$new(
  		...
  		total = setup$nmcmc, clear = FALSE, width= 60)
	for (m in 2:setup$nmcmc){
		...
		for (i in 1:setup$nexp){
			...
			if (setup$models[[i]]$stochastic){
				pred_curr[i] = eval(setup$models[[i]], tran_unif(theta[m,,], setup$bounds_mat, names(setup$bounds)), TRUE)
			}
			if ((setup$models[[i]]$nd > 0) | (setup$models[[i]]$stochastic)){
				for (t in 1:setup$ntemps){
					llik_curr[i, t] = llik(setup$models[[i]], setup$ys[[i]]-discrep_curr[[i]][t,], pred_curr[[i]][t, ], marg_lik_cov_curr[[i]][[t]])
				}
			}
		}
		...
		good_values = setup$checkConstraints(tran_unif(theta_cand, setup$bounds_mat, names(setup$bounds)), setup$bounds)
		...
		if (any(good_values)){
			...
			for (i in 1:setup$nexp){
				pred_cand[[i]][good_values,] = eval(setup.models[[i]], train_unif(theta_cand[good_values,], setup$bounds_mat, names(setup$bounds)), TRUE)
				for (t in 1:setup$ntemps){
					llik_cand[i,t] = llik(setup.models[[ii]], setup$ys[[i]]-discrep_curr[[i]][t,], pred_cand[[i]][t,], marg_lik_cov_curr[[i]][[t]])
				}
			}
		}
		...
		if (mod(m, setup$decor)==0){
			for (k in 1:setup$p){
				...
				if (any(good_values)){
					llik_cand[, good_values] = 0
					for (i in 1:setup$nexp){
						pred_cand[[i]][good_values,] = eval(setup$models[[i]], tran_unif(theta_cand[good_values,], setup$bounds_mat, names(setup$bounds)), TRUE)
						for (t in 1:setup$ntemps){
							llik_cand[i,t] = llik(setup$models[[i]], setup$ys[[i]]-discrep_curr[[i]][t,], pred_cand[[i]][t,], marg_lik_cov_curr[[i]][[t]])
		...
		for (i in 1:setup$nexp){
			if (setup$models[[i]]$s2 == 'gibbs'){
				# gibbs update s2
				 ...
				 log_s2[[i]][m,] = log(1/rgamma(itl_mat[[i]]*(setup$ny_s2[[i]]/2 + setup$ig_a[[i]] + 1) - 1, 1/(itl_mat[[i]]*(setup$ig_b[[i]] + dev_sq/2))))
				 for (t in 1:setup$ntemps){
				...
				for (t in 1:setup$ntemps){
					...
					llik_candi[t] = llik(setup$models[[i]], setup$ys[[i]]-discrep_curr[[i]][t,], pred_curr[[i]][t,], marg_lik_cov_candi[[t]])
				}
				...
				alpha_s2 = alpha_s2 - setup$itl * rowSums(setup$s2_prior_kern[[i]](exp(log_s2[[i]][m-1,]), setup$ig_a[[i]], setup$ig_b[[i]]))
